...
statement
    ...
    | CREATE AMDS INDEX indexIdentifier=identifier ON
        tableIdentifier '(' column=identifier ')'                      #createAMDSIndex
    | DROP AMDS INDEX indexIdentifier=identifier ON tableIdentifier    #dropAMDSIndex
    | SHOW AMDS INDEXES                                                #showAMDSIndexes
    | CREATE MBT INDEX indexIdentifier=identifier ON
        tableIdentifier function=metricSimilarityFunction '(' column=identifier ')'                      #createMBTIndex
    | DROP MBT INDEX indexIdentifier=identifier ON tableIdentifier     #dropMBTIndex
    | SHOW MBT INDEXES                                                 #showMBTIndexes
    | CREATE ODB INDEX indexIdentifier=identifier ON
        tableIdentifier '(' columnDouble=identifier ',' columnString=identifier ',' columnMetricM=identifier ',' columnMetricMaxDis=identifier ')'                      #createODBIndex
    | DROP ODB INDEX indexIdentifier=identifier ON tableIdentifier     #dropODBIndex
    | SHOW ODB INDEXES                                                 #showODBIndexes
...
valueExpression
    ...
    | leftTable=primaryExpression IN METRICRANGE '(' center=metricPointExpression
           ',' radius=number ')'                                                             #metricRange
    | amdsSimilarityExpression KNN count=number                                              #amdsSimilarityWithKNN
    | leftTable=primaryExpression IN AMDSRANGE '(' center=amdsPointExpression
           ',' radius=number ')'                                                             #amdsRange
    | mbtSimilarityExpression KNN count=number                                               #mbtSimilarityWithKNN
    | leftTable=primaryExpression IN MBTRANGE function=metricSimilarityFunction '(' center=mbtPointExpression
           ',' radius=number ')'                                                             #mbtRange
    | leftTable=primaryExpression IN ODBKNN '(' center=odbPointExpression
           ',' count=number ')'                                                              #odbSimilarityWithKNN
    | leftTable=primaryExpression IN ODBRANGE '(' center=odbPointExpression
           ',' radius=number ')'                                                             #odbRange
...
metricSimilarityExpression
    ...
    | function=metricSimilarityFunction '(' leftMetricPoint=metricPointExpression ',' rightTable=primaryExpression ')'
    | function=metricSimilarityFunction '(' leftTable=primaryExpression ',' rightMetricPoint=metricPointExpression ')'
    ;

metricPointExpression
    : METRICPOINT '(' mcoords=metricCoords ')'
    ;
...
amdsSimilarityExpression
    : function=metricSimilarityFunction '(' leftTable=primaryExpression ',' rightTable=primaryExpression ')'
    | function=metricSimilarityFunction '(' leftMetricPoint=amdsPointExpression ',' rightTable=primaryExpression ')'
    | function=metricSimilarityFunction '(' leftTable=primaryExpression ',' rightMetricPoint=amdsPointExpression ')'
    ;

amdsPointExpression
    : AMDSPOINT '(' mcoords=metricCoords ')'
    ;

mbtSimilarityExpression
    : function=metricSimilarityFunction '(' leftTable=primaryExpression ',' rightTable=primaryExpression ')'
    | function=metricSimilarityFunction '(' leftMetricPoint=mbtPointExpression ',' rightTable=primaryExpression ')'
    | function=metricSimilarityFunction '(' leftTable=primaryExpression ',' rightMetricPoint=mbtPointExpression ')'
    ;

mbtPointExpression
    : MBTPOINT '(' mcoords=metricCoords ')'
    ;

odbSimilarityExpression
    : '(' leftTable=primaryExpression ',' rightTable=primaryExpression ')'
    | '(' leftMetricPoint=odbPointExpression ',' rightTable=primaryExpression ')'
    | '(' leftTable=primaryExpression ',' rightMetricPoint=odbPointExpression ')'
    ;

odbPointExpression
    : ODBPOINT '(' obtcoords=obtCoords ')' ',' METRICM '('metricm =  metricM ')' ',' QUERYM '('querym =  metricM ')'
    ;


obtCoords
    :  coords+=mixedItem ('|' coords+=mixedItem)*
    ;
...
//metricExpression
//    : METRICPOINT '(' points+=metricPointExpression (',' points+=metricPointExpression)* ')' #metricDefault
//    ;
